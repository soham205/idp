...
import { AsyncFunction, IRouterElement } from "./interfaces";
...
function getPath(routeComponent: IRouterElement) {
...
function getCallback(routeComponent: IRouterElement) {
  if (!routeComponent.path) {
    pc.red(
...
const staq_cms_routeResolve = {
  resolveRoute: (routeTable: IRouterElement[]) => {
    const router = Router();
