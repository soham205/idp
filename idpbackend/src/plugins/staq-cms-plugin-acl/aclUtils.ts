...
const matchRouteToResource = (route: string, resource: string) => {
  ...
  for (
    let routeArrayCounter = 0;
    routeArrayCounter < routeArray.length;
    routeArrayCounter++
  ) {
    ...
    if (
      !routeArray[routeArrayCounter].match(
        createRegexFromResource(resourceArray[routeArrayCounter])
      )
    )
      return false;
  }
...
export const aclUtils = {
  mapPolicyToGroup: (
    policies: IGroupWiseAcl[]
  ): Map<string, IPoliciyElement[]> => {
  ...
  getRolesFromRequest: (
    req: IDecodedTokenRequestObject,
    defaultRole: string
  ) => {
